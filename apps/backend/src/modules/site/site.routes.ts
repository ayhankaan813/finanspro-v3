...
export async function siteRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { id: string }; Querystring: { page?: string; limit?: string } }>(
    ...
    async (request, reply) => {
      ...
      const result = await siteService.getTransactions(request.params.id, { page, limit });
  ...
  app.get<{ Params: { id: string }; Querystring: { page?: string; limit?: string } }>(
    ...
    async (request, reply) => {
      const page = Math.max(1, parseInt(request.query.page || '1', 10) || 1);
      const limit = Math.min(100, Math.max(1, parseInt(request.query.limit || '20', 10) || 20));
      const result = await siteService.getLedgerEntries(request.params.id, { page, limit });
